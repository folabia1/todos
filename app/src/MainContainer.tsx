...
export default () => {
  ...
  const [toggleCompletedForTodoItems] = api.endpoints.toggleCompletedForTodoItems.useMutation({});
  ...
  return (
    <MainContainer>
      ...
      {todoItems && todoItems.data.length > 1 && (
        <button
          onClick={() => {
            toggleCompletedForTodoItems({ ids: todoItems.data.map((todoItem) => todoItem.id), completed: !allCompleted })
              .unwrap()
              .finally(() => {
                refetch();
              });
